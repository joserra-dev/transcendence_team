...
class EmailService:
    ...
    @classmethod
    def welcome(cls, destinatario: str, asunto: str = "HEMEN-GO - Bienvenido"):
        login_url = f"{current_app.config.get('FRONTEND_URL', 'http://localhost:8001').rstrip('/')}/auth/login-client"
        html_content = render_template(
            'email/bienvenida.html',
            nombre=destinatario,
            email=destinatario,
            enlace_verificacion=login_url
        )
